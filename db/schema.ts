...
export const missions = pgTable("missions", {
  ...
  seeTemplateKey: text("see_template_key"),
  seeTemplateSnapshot: jsonb("see_template_snapshot").$type<Record<string, unknown>>(),
  seeServiceLevel: text("see_service_level"),
  seeWindowHours: integer("see_window_hours"),
  seeEarliestVisitAt: timestamp("see_earliest_visit_at", { withTimezone: true }),
  seeDeadlineAt: timestamp("see_deadline_at", { withTimezone: true }),
  seeAssignmentCutoffAt: timestamp("see_assignment_cutoff_at", { withTimezone: true }),
  seeFundedAt: timestamp("see_funded_at", { withTimezone: true }),
  seeBasePayoutCents: integer("see_base_payout_cents"),
  seePayoutCapCents: integer("see_payout_cap_cents"),
  seeAcceptedPayoutCents: integer("see_accepted_payout_cents"),
  seeBoostPaused: boolean("see_boost_paused").notNull().default(false),
  seeOfferVersion: integer("see_offer_version").notNull().default(0),
  seeExpiredAt: timestamp("see_expired_at", { withTimezone: true }),
  seeReportStatus: text("see_report_status"),
  seeReportRevision: integer("see_report_revision").notNull().default(0),
  seeReportReleasedAt: timestamp("see_report_released_at", { withTimezone: true }),
  ...
  check("missions_result_upload_token_count_check", sql`${table.resultUploadTokenCount} >= 0 AND ${table.resultUploadTokenCount} <= 150`),
...
export const seeMissionDrafts = pgTable("see_mission_drafts", {
  missionId: uuid("mission_id").primaryKey().references(() => missions.id, { onDelete: "cascade" }),
  scoutId: uuid("scout_id").notNull().references(() => users.id),
  data: jsonb("data").$type<import("@/lib/see-report-types").SeeDraft>().notNull(),
  updatedAt: timestamp("updated_at", { withTimezone: true }).defaultNow().notNull(),
});
export const seeReports = pgTable("see_reports", {
  id: uuid("id").defaultRandom().primaryKey(),
  missionId: uuid("mission_id").notNull().references(() => missions.id, { onDelete: "cascade" }),
  revision: integer("revision").notNull(),
  status: text("status").notNull().default("pending"),
  snapshot: jsonb("snapshot").$type<import("@/lib/see-report-types").SeeReportSnapshot>().notNull(),
  storagePath: text("storage_path"), error: text("error"), attempts: integer("attempts").notNull().default(0),
  reviewedBy: uuid("reviewed_by").references(() => users.id), reviewNote: text("review_note"),
  createdAt: timestamp("created_at", { withTimezone: true }).defaultNow().notNull(),
  updatedAt: timestamp("updated_at", { withTimezone: true }).defaultNow().notNull(),
}, table => [uniqueIndex("see_reports_mission_id_revision_key").on(table.missionId, table.revision)]);
export const seeOfferEvents = pgTable("see_offer_events", {
  id: uuid("id").defaultRandom().primaryKey(), missionId: uuid("mission_id").notNull().references(() => missions.id, { onDelete: "cascade" }),
