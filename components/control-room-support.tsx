...
import { adminApproveCustomerSupportRefund, adminProposeCustomerSupportResolution, adminReplyToCustomerSupport } from "@/app/actions/support";
...
export type AdminSupportTicketView = {
  ...
  financialApprovalPending: boolean;
  proposedByCurrentAdmin: boolean;
  createdAt: string;
  updatedAt: string;
    ...
    <section className="control-section">
      ...
      {tickets.length ? <div className="admin-support-list">{tickets.map((ticket) => {
        ...
        return <article className="admin-support-ticket" key={ticket.id}>
          <div className="support-ticket-heading"><div><small>{customerSupportReasonLabel(ticket.reason)} · {ticket.customer}</small><h3>{ticket.missionTitle ?? "General customer support"}</h3><p>{ticket.customerEmail} · Opened {new Date(ticket.createdAt).toLocaleString()} · Ticket {ticket.id.slice(0, 8).toUpperCase()}</p></div><span className={`status ${ticket.status === "awaiting_customer" || ticket.financialApprovalPending ? "warning-status" : ticket.status === "closed" ? "" : "muted-status"}`}>{ticket.financialApprovalPending ? "Awaiting second admin" : statusLabel(ticket.status)}</span></div>
          ...
          {ticket.resolutionType && ticket.resolutionNote && <div className={`support-resolution-card ${ticket.status === "closed" ? "approved" : "pending"}`}><span><IconReceiptRefund size={23} /></span><div><small>{ticket.status === "closed" ? "Customer-approved resolution" : ticket.financialApprovalPending ? "High-value refund awaiting second administrator" : "Resolution awaiting customer"}</small><h4>{customerSupportResolutionLabel(ticket.resolutionType)} · {money(ticket.resolutionAmountCents)}</h4><p>{ticket.resolutionNote}</p>{ticket.customerDecision === "needs_review" && <p>The customer requested another review after this proposal.</p>}{ticket.financialApprovalPending && <><button className="button button-small" disabled={pending || ticket.proposedByCurrentAdmin} onClick={() => run(() => adminApproveCustomerSupportRefund(ticket.id))}>Approve financial review</button><small>{ticket.proposedByCurrentAdmin ? "A different administrator must approve this refund." : "Your approval will send the unchanged resolution to the customer."}</small></>}</div></div>}
