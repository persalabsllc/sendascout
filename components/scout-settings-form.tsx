...
export function ScoutSettingsForm({ initial, headshotUrl, scoutId }: { initial: ScoutSettingsInput; headshotUrl: string | null; scoutId: string }) {
  ...
  return <div className="settings-form">
    <div className="field-row" id="scout-contact"><label className="field"><span>Legal first name</span><input value={value.firstName} autoComplete="given-name" onChange={(event) => set("firstName", event.target.value)} /></label><label className="field"><span>Legal last name</span><input value={value.lastName} autoComplete="family-name" onChange={(event) => set("lastName", event.target.value)} /></label></div>
    <label className="field"><span>Mobile number</span><input type="tel" autoComplete="tel" placeholder="555-123-4567" value={value.phone} onChange={(event) => set("phone", event.target.value)} /></label>
    <label className="check notification-check"><input type="checkbox" checked={value.verificationConsent} onChange={(event) => set("verificationConsent", event.target.checked)} /><span><strong>Identity and payout verification</strong><small>I am at least 18 and agree that Stripe may verify my identity and payout information before I can claim missions.</small></span></label>
    <div className="headshot-editor" id="scout-headshot"><div className="headshot-preview">{photoUrl ? <Image src={photoUrl} alt="Your Scout profile photo" width={112} height={112} unoptimized /> : <IconCamera size={34} />}</div><div><h3>Headshot photo</h3><p>Use a clear, current JPG, PNG, or WEBP photo of your face so customers know whom to expect. You may update it between missions; Stripe payout identity verification remains separate.</p><label className="button button-ghost button-small">{photoPending ? "Uploading…" : photoUrl ? "Change photo" : "Add photo"}<input type="file" accept="image/jpeg,image/png,image/webp" disabled={photoPending} onChange={(event) => { const file = event.target.files?.[0]; if (file) void uploadHeadshot(file); }} /></label></div></div>
    <div className="field-row" id="scout-service"><label className="field"><span>Home ZIP code</span><input inputMode="numeric" value={value.homeZip} onChange={(event) => set("homeZip", event.target.value)} /></label><label className="field"><span>Delivery zone</span><select value={value.serviceRadiusMiles} onChange={(event) => set("serviceRadiusMiles", Number(event.target.value))}><option value={10}>Within 10 miles</option><option value={25}>Within 25 miles</option><option value={50}>Within 50 miles</option><option value={75}>Within 75 miles</option></select></label></div>
