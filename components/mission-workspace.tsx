      ...
      <div className="mission-shell">
        ...
        <div className="mission-work-grid">
          <section className="mission-column">
            ...
            {role === "scout" && !assigned && !canClaim && claimRequirement && <article className="mission-panel claim-panel claim-locked-panel"><IconShieldCheck size={28} /><div><h2>You can review this mission now</h2><p>{claimRequirement.message} Claiming remains locked until your Scout checklist is complete, and the mission may be accepted by another ready Scout first.</p></div>{claimRequirement.href && <Link className="button button-ghost" href={claimRequirement.href}>{claimRequirement.label ?? "Continue onboarding"}</Link>}</article>}
          ...
          <aside className="mission-column">
            ...
            {role === "scout" ? <article className="mission-panel tracking-panel mission-map-panel">
              ...
              <div className="map-footer"><small>{role === "scout" && !assigned ? "Planning view · the exact address unlocks after claiming" : mission.type === "move" ? "Pickup and drop-off route" : "Mission location"}</small>{mission.directionsUrl && <a href={mission.directionsUrl} target="_blank" rel="noreferrer">Open in Google Maps <IconNavigation size={15} /></a>}</div>
...
function MeetTimerPanel({ role, mission, clock, pending, extend }: { role: "customer" | "scout" | "admin"; mission: MissionView; clock: number | null; pending: boolean; extend: () => void }) {
  ...
  const cap = role === "scout"
    ? money(mission.maximumScoutPayoutCents ?? mission.scoutPayoutCents)
    : money(mission.maximumCustomerPriceCents ?? 2900);
  return <article className="mission-panel timer-panel">
    ...
    <div className="timer-display"><small>{mission.billableEndedAt ? "Final verified time" : mission.billableStartedAt ? "Verified timer running" : "Timer has not started"}</small><strong>{formatElapsed(elapsedSeconds)}</strong><span>{mission.meetAuthorizedMinutes / 60} hour{mission.meetAuthorizedMinutes === 60 ? "" : "s"} authorized · {cap} {role === "scout" ? "maximum earnings" : "maximum"}</span></div>
    ...
    {mission.billableEndedAt && <p className="timer-note">{role === "scout" ? `Final billable time: ${mission.chargedMinutes ?? 60} minutes.` : `Customer charged for ${mission.chargedMinutes ?? 60} minutes.`} Actual verified presence: {mission.billableMinutes ?? 0} minutes.</p>}
