    ...
    <div className="control-shell">
      ...
      <section className="control-section">
        ...
        {missions.length ? <div className="control-table mission-admin-list">{missions.map((mission) => <article key={mission.id}>
          ...
          <div className="control-actions">
            ...
            {!['completed', 'cancelled', 'disputed'].includes(mission.status) && <button className="danger-link" disabled={pending} onClick={() => run(() => adminSetMissionStatus(mission.id, "cancelled"))}>Cancel</button>}
      ...
      <section className="control-section">
        ...
        {cases.length ? <div className="case-list">{cases.map((item) => <article key={item.id}>
          ...
          {item.status === "open" ? <CaseResolutionControls item={item} pending={pending} resolve={(resolution, notes, refund, payout) => run(() => adminResolveMissionCase(item.id, resolution, notes, refund, payout))} /> : <div className="case-resolution"><strong>Resolution: {titleCase(item.resolution ?? "resolved")}</strong><p>{item.adminNotes}</p><small>Refund recorded: {money(item.refundAmountCents)} · Scout payout recorded: {money(item.payoutAmountCents)}</small></div>}
...
function CaseResolutionControls({ item, pending, resolve }: { item: CaseRow; pending: boolean; resolve: (resolution: "resume" | "cancel" | "complete" | "hold", notes: string, refund: number, payout: number) => void }) {
  const [resolution, setResolution] = useState<"resume" | "cancel" | "complete" | "hold">("hold");
  const [notes, setNotes] = useState("");
  const [refundInput, setRefundInput] = useState("0.00");
  ...
  return <div className="case-controls">{item.adminNotes && <div className="case-resolution"><strong>Saved review notes</strong><p>{item.adminNotes}</p></div>}<label><span>Outcome</span><select value={resolution} onChange={(event) => setResolution(event.target.value as typeof resolution)}><option value="hold">Keep paused</option><option value="resume">Resume mission</option><option value="cancel">Cancel mission</option><option value="complete">Mark complete</option></select></label><label><span>Refund · remaining maximum {money(item.customerPriceCents)}</span><input inputMode="decimal" type="number" min="0" max={item.customerPriceCents / 100} step="1" value={refundInput} onChange={(event) => setRefundInput(event.target.value)} onBlur={() => setRefundInput(normalizeMoneyInput(refundInput))} /></label><label><span>Scout payout · remaining maximum {money(item.scoutPayoutCents)}</span><input inputMode="decimal" type="number" min="0" max={item.scoutPayoutCents / 100} step="1" value={payoutInput} onChange={(event) => setPayoutInput(event.target.value)} onBlur={() => setPayoutInput(normalizeMoneyInput(payoutInput))} /></label><label className="case-notes"><span>Internal review note · required</span><textarea rows={3} maxLength={3000} placeholder="Briefly explain the decision and evidence reviewed." value={notes} onChange={(event) => setNotes(event.target.value)} /></label>{validationError && <p className="case-validation-error" role="alert">{validationError}</p>}<button disabled={pending} onClick={submit}>{pending ? resolution === "hold" ? "Saving…" : "Resolving…" : resolution === "hold" ? "Save & keep paused" : "Resolve case"}</button><small>{resolution === "hold" ? "Keeping a case paused saves the review note but does not close the case or record money. " : ""}Use the amount in dollars. Keyboard entry is supported and the arrow controls change the value by $1. Financial amounts are recorded now and will execute automatically only after Stripe is activated.</small></div>;
}
