...
import { IconBuildingStore, IconCalendarRepeat, IconDashboard, IconLifebuoy, IconSettings, IconTargetArrow, IconUser, IconWallet } from "@tabler/icons-react";
import { Brand } from "./brand";
import { MobileDashboardNav } from "./mobile-dashboard-nav";

export function CustomerDashboardShell({ active, name, children }: { active: "overview" | "payments" | "profile" | "saved" | "business" | "support"; name: string; children: React.ReactNode }) {
  ...
  return <main className="dashboard-page"><aside className="dash-sidebar"><Brand /><nav>
    ...
    <Link className={active === "saved" ? "active" : ""} href="/dashboard/customer/saved"><IconCalendarRepeat size={20} /> Saved &amp; recurring</Link>
    <Link className={active === "business" ? "active" : ""} href="/dashboard/customer/business"><IconBuildingStore size={20} /> Business</Link>
