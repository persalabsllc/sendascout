...
const emptyMission: MissionInput = {
  ...
  addMoveLeg: false, bundleDropoffName: "", bundleDropoffAddress: "", bundleDropoffAddressLine2: "", bundleDropoffCity: "", bundleDropoffState: "NC", bundleDropoffZip: "", bundleDeliveryInstructions: "", bundleTitle: "", bundleInstructions: "", bundleLargeItem: false,
...
export function OnboardingForm({ mode, initialMissionType = "see", initialMissionAddress, initialPhone = "", initialMission, preferredScouts = [] }: { mode: Mode; initialMissionType?: MissionInput["type"]; initialMissionAddress?: Pick<MissionInput, "address" | "addressLine2" | "city" | "zip">; initialPhone?: string; initialMission?: Partial<MissionInput>; preferredScouts?: PreferredScoutOption[] }) {
  const { user } = useUser();
...
function CustomerStep({ step, value, setValue, quote, preferredScouts }: { step: number; value: MissionInput; setValue: React.Dispatch<React.SetStateAction<MissionInput>>; quote: MissionCreationQuote | null; preferredScouts: PreferredScoutOption[] }) {
  ...
  if (step === 1) return <><Field label={value.type === "meet" ? "Meeting location" : "Inspection location"} placeholder="Street address" value={value.address} onChange={(event) => set("address", event.target.value)} /><Field label="Apartment, suite, etc. (optional)" required={false} value={value.addressLine2} onChange={(event) => set("addressLine2", event.target.value)} /><div className="field-row"><Field label="City" placeholder="New Bern" value={value.city} onChange={(event) => set("city", event.target.value)} /><Field label="ZIP code" placeholder="28560" inputMode="numeric" value={value.zip} onChange={(event) => set("zip", event.target.value)} /></div><Field label={value.type === "meet" ? "When should the Scout arrive?" : "When do you need a Scout?"} type="datetime-local" readOnly={Boolean(value.recurrenceScheduleId)} value={value.scheduledFor} onChange={(event) => set("scheduledFor", event.target.value)} /></>;
  ...
  return <div className="review-box"><Review label="Mission" value={value.addMoveLeg ? `${titleCase(value.type)} It + Move It · 2 parts` : `${titleCase(value.type)} It`} /><Review label="Location" value={`${value.city || "City"}, NC ${value.zip}`} /><Review label="Timing" value={value.scheduledFor ? formatLocalInput(value.scheduledFor) : "As soon as possible"} />{value.addMoveLeg && <Review label="Follow-up delivery" value={`${value.bundleDropoffName} · ${formatMovePlace(value.bundleDropoffAddress, value.bundleDropoffCity, value.bundleDropoffState, value.bundleDropoffZip)}`} />}<Review label={value.recurrenceScheduleId ? "Recurring occurrence" : "Repeat schedule"} value={value.recurrenceScheduleId ? "One reviewed occurrence · no automatic charge" : recurrenceLabel(value.recurrence)} />{value.preferredScoutId && <Review label="Scout preference" value="Offer to your preferred Scout for 1 hour" />}{value.enhancedReport && <Review label={value.addMoveLeg ? "Enhanced report · Meet It part" : "Enhanced report"} value={`${value.checklistItems.length} required item${value.checklistItems.length === 1 ? "" : "s"}`} />}{quote?.itemized.map((line) => <Review key={line.label} label={line.label} value={money(line.customerPriceCents)} />)}<Review label="Mission total" value={money(quote?.totalCustomerPriceCents ?? 2900)} />{value.type === "meet" && <Review label="Authorized maximum" value={`${value.meetAuthorizedMinutes / 60} hour${value.meetAuthorizedMinutes === 60 ? "" : "s"} · ${money(quote?.maximumCustomerPriceCents ?? 2900)} for part 1`} />}<p>{value.type === "meet" ? "Verified check-in opens five minutes before the scheduled time. Billing starts only after onsite GPS verification; additional time is billed in 15-minute increments and cannot exceed your approved maximum." : "Includes up to 20 minutes onsite and written findings. Photos or video can be included when the mission calls for them."}</p>{value.addMoveLeg && <p>One eligible Scout claims both ordered parts. The $4 multi-mission discount comes from Send a Scout’s portion; Scout pay is not reduced.</p>}<p>You won’t be charged yet. Payment authorization activates before the soft launch.</p></div>;
}
...
function recurrenceLabel(value: MissionInput["recurrence"]) { return value === "weekly" ? "Every week · Eastern Time" : value === "biweekly" ? "Every 2 weeks · Eastern Time" : value === "monthly" ? "Every month · Eastern Time" : "One time"; }
...
function formatLocalInput(value: string) {
  return formatEasternDateTime(easternLocalDateTimeToUtc(value)).replace(/\s(?:EST|EDT)$/, " ET");
}
