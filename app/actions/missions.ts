  ...
  try {
    ...
    if (mission.type === "meet") {
      ...
      if (nextStatus === "en_route" && !meetActionIsAvailable(mission.scheduledFor, "en_route", now)) {
        throw new Error(`Travel status opens 30 minutes before the appointment, at ${formatEasternDateTime(meetActionOpensAt(mission.scheduledFor, "en_route"))}.`);
      }
      if (nextStatus === "onsite" && !meetActionIsAvailable(mission.scheduledFor, "onsite", now)) {
        throw new Error(`Verified check-in opens five minutes before the appointment, at ${formatEasternDateTime(meetActionOpensAt(mission.scheduledFor, "onsite"))}.`);
      }
    }
...
export async function verifyMissionDeliveryPin(id: string, pin: string): Promise<Result> {
  try {
    ...
    if (isDeliveryPinLocked(mission.deliveryPinLockedUntil, now)) {
      throw new Error(`PIN entry is temporarily locked until ${formatEasternDateTime(mission.deliveryPinLockedUntil!)}. Contact support if the recipient cannot provide the code.`);
    }
