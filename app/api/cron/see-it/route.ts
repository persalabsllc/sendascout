...
export async function GET(request:Request){
 ...
 for(const [name,operation] of [["fulfillment",runSeeFulfillment],["reports",reconcileSeeReports]] as const){try{results[name]=await operation();}catch(error){failed=true;console.error(`See It ${name} worker failed`,error);results[name]={error:"Worker failed; retry required"};}}
