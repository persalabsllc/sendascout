...
import { flushPlatformActivityAlerts } from "@/lib/platform-activity-alerts";
...
export async function GET(request:Request){
 ...
 for(const [name,operation] of [["payments",reconcilePendingBookingPayments],["fulfillment",runSeeFulfillment],["reports",reconcileSeeReports],["activityAlerts",flushPlatformActivityAlerts]] as const){try{results[name]=await operation();}catch(error){failed=true;console.error(`See It ${name} worker failed`,error);results[name]={error:"Worker failed; retry required"};}}
