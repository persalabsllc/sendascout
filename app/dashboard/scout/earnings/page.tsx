...
export default async function ScoutEarningsPage() {
  ...
  const rows = await db.select().from(missions).where(and(eq(missions.scoutId, user.id), isNull(missions.archivedAt))).orderBy(desc(missions.completedAt), desc(missions.createdAt));
