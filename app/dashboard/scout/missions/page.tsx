...
function MissionSection({ title, empty, rows, emptyAction }: { title: string; empty: string; rows: (typeof missions.$inferSelect)[]; emptyAction?: { href: string; label: string } }) {
  return <section className="dash-section"><div className="dash-section-title"><div><h2>{title}</h2><p>Each amount shown is what you&apos;ll earn for completing that mission.</p></div></div>{rows.length ? <div className="mission-list scout-list">{rows.map((mission) => {
    const Icon = mission.type === "see" ? IconCamera : mission.type === "move" ? IconRoute : IconClock;
    return <Link className="mission-list-row" href={`/dashboard/missions/${mission.id}`} key={mission.id}><span className="list-icon"><Icon size={22} /></span><div className="list-main"><small>{mission.type === "see" ? "See It" : mission.type === "move" ? "Move It" : "Meet It"}</small><strong>{mission.title}</strong><span><IconMapPin size={14} /> {mission.city}, {mission.state} {mission.zip}</span></div><div className="payout"><small>You&apos;ll earn</small><strong>{money(mission.scoutPayoutCents)}</strong></div><span className="claim-button">{mission.status === "open" ? "Review" : label(mission.status)}</span></Link>;
