...
export default async function ScoutSettingsPage() {
  ...
  return <ScoutDashboardShell active="settings" name={name}><div className="dash-welcome simple-title"><div><span className="kicker">Scout profile</span><h1>Profile &amp; mission preferences</h1><p>Help customers recognize who is arriving, then choose the missions you want to see.</p></div></div><section className="dash-section settings-section"><div className="dash-section-title"><div><h2>Public Scout profile</h2><p>Your first name, photo, completed mission count and rating appear only to customers whose mission you accept.</p></div></div><ScoutSettingsForm headshotUrl={profile.headshotPath ? `/api/scout-headshot?scoutId=${encodeURIComponent(user.id)}` : null} initial={{ homeZip: profile.homeZip ?? "", serviceRadiusMiles: profile.serviceRadiusMiles, vehicleType: profile.vehicleType ?? "", canSee: profile.canSee, canMove: profile.canMove, canMeet: profile.canMeet, emailNotificationsEnabled: user.emailNotificationsEnabled }} /></section></ScoutDashboardShell>;
}
